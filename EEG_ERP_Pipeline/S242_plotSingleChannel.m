...
% Single-channel ERP plotting with baseline correction
    ...
    if ~isempty(rowIdxSubset)
        ...
        [~, idx] = S230_getROILabelAndIndices(channelLabel, meta);
        rowIdx   = idx(1);
    end
 
    if strlength(channelLabel) == 0
        allLabels    = S230_getROILabelAndIndices('all');
        channelLabel = allLabels(rowIdx);
    end
 
    condInfo = {
        'Exp_noSP',      getOpt(cfg,'Exp_noSP',false),     [0.6, 0,   0.6], 'Expected w/o SP';
        'Unexp_noSP',    getOpt(cfg,'Unexp_noSP',false),   [0,   0.6, 0.3], 'Unexpected w/o SP';
        'Diff_noSP',     getOpt(cfg,'Diff_noSP',false),    [0.85,0.3, 0.1], 'Unexp-Exp w/o. SP';
        'Exp_withSP',    getOpt(cfg,'Exp_withSP',false),   [0.2, 0.5, 0.9], 'Expected w SP';
        'Unexp_withSP',  getOpt(cfg,'Unexp_withSP',false), [0.9, 0.6, 0.2], 'Unexpected w SP';
        'Diff_withSP',   getOpt(cfg,'Diff_withSP',false),  [0,   0.6, 0  ], 'Unexp-Exp w. SP';
        'Atonal',        getOpt(cfg,'Atonal',false),       [0.5, 0.5, 0.5], 'Atonal'
    };
    ...
    if splitPanels
        ...
        hFig = figure('Name','Single Channel - Individual and SEM','Color','w','Visible','on');
        if plotCloseUp
    ...
    for p = 1:nPanels
        ...
        for c = 1:numel(condsToPlot)
            ...
            tr = prepareTraces(Extract_Data_Indiv.(condsToPlot{c}), ...
                               rowIdx, timeInfo.baselineIdx_old);
    ...
    figureTitle = {sprintf('Single Channel: %s (%s)',channelLabel,roiTitle); ...
...
% Keep identical to the copies in S241 and S243.
